...
const resultHalfLife = document.querySelector(
...
btnCalcHalfLife.addEventListener("click", (event) => {
  ...
  const result = resultHalfLife.innerHTML
    .match(/[-+]?[0-9]*\.?[0-9]+/g)
    .join("")
...
btnCalcToxicity.addEventListener("click", (event) => {
  const ingestionIntervals = document.querySelectorAll(
    ".nomogram__interval-after-ingestion"
  )
  const paracetamolConcentrationIntervals = document.querySelectorAll(
  ...
  const result = document.querySelector(".nomogram-result__text").attributes[
    "data-result"
  ].value
  gtag("event", event.target.attributes["data-text"].value, {
    event_category: "button",
